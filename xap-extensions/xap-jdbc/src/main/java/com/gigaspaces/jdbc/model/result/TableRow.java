...
public class TableRow implements Comparable<TableRow> {
    ...
    public TableRow(IQueryColumn[] columns, Object[] values) {
        this(columns, values, new OrderColumn[0], new Object[0]);
    }
    ...
    TableRow(IEntryPacket entryPacket, ConcreteTableContainer tableContainer) {
        ...
        if (tableContainer.hasAggregationFunctions() && entryPacket instanceof QueryEntryPacket) {
            ...
            for (int i = 0; i < this.columns.length; i++) {
                IQueryColumn IQueryColumn = this.columns[i];
                if (IQueryColumn.isUUID()) {
                    values[i] = entryPacket.getUID();
                } else if (entryPacket.getTypeDescriptor().getIdPropertyName().equalsIgnoreCase(IQueryColumn.getName())) {
                    values[i] = entryPacket.getID();
                } else {
                    values[i] = entryPacket.getPropertyValue(IQueryColumn.getName());
                }
            }
        }
        ...
        for (int i = 0; i < orderColumns.size(); i++) {
            ...
            if (orderColumn.isUUID()) {
                orderValues[i] = entryPacket.getUID();
            } else if (entryPacket.getTypeDescriptor().getIdPropertyName().equalsIgnoreCase(orderColumn.getName())) {
                orderValues[i] = entryPacket.getID();
            } else {
                orderValues[i] = entryPacket.getPropertyValue(orderColumn.getName());
            }
        }
    }

    TableRow(List<IQueryColumn> columns, List<OrderColumn> orderColumns) {
        this.columns = columns.toArray(new IQueryColumn[0]);
        values = new Object[this.columns.length];
        for (int i = 0; i < columns.size(); i++) {
            values[i] = columns.get(i).getCurrentValue();
        }
