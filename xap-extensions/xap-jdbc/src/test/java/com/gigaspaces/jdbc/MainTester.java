...
//            execute(statement, String.format("SELECT * FROM %s WHERE", MyPojo.class.getName()));
//            execute(statement, String.format("SELECT id FROM (SELECT A.id, B.id, A.name, B.name FROM %s A INNER JOIN %s B ON A.name=B.name) as C", MyPojo.class.getName(), MyPojo.class.getName()));
            execute(statement, String.format("SELECT name FROM (SELECT id,age, name FROM %s) where (name = 'Adler' or name = 'Adam') AND age > 30", MyPojo.class.getName()));
