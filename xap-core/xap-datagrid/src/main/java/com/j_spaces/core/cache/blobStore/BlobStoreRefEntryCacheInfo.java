        ...
        synchronized (getStateLockObject()) {
            ...
            if (removed || isDeleted() || isPhantom()) {
                ...
                if (cacheManager.isOffHeapOptimizationEnabled()) {
                    OffHeapIndexesValuesHandler.delete(this, cacheManager.getOffHeapByteCounter());
                }
                ...
                if (!isWrittenToBlobStore()) {
                    ...
                    if (cacheManager.isOffHeapOptimizationEnabled()) {
                        try {
                            setOffHeapAddress(OffHeapIndexesValuesHandler.allocate(((BlobStoreEntryLayout) getEntryLayout(cacheManager)).getIndexValuesBytes(cacheManager), getOffHeapAddress(), cacheManager.getOffHeapByteCounter()));
                        } catch (IOException e) {
                    ...
                    if (cacheManager.isOffHeapOptimizationEnabled()) {
                        try {
                            OffHeapIndexesValuesHandler.update(this, ((BlobStoreEntryLayout) getEntryLayout(cacheManager)).getIndexValuesBytes(cacheManager), cacheManager.getOffHeapByteCounter());
                        } catch (IOException e) {
            ...
            if (isDeleted() && !entry.isPhantom()) {
                ...
                if (!isWrittenToBlobStore()) {
                    ...
                    if (cacheManager.isOffHeapOptimizationEnabled() && !isPhantom()) {
                        setOffHeapAddress(OffHeapIndexesValuesHandler.allocate(entryLayout.getIndexValuesBytes(cacheManager), getOffHeapAddress(), cacheManager.getOffHeapByteCounter()));
                    }
                    _blobStorePosition = cacheManager.getBlobStoreStorageHandler().add(getStorageKey_impl(), entryLayout, BlobStoreObjectType.DATA);
                } else {
                    if (cacheManager.isOffHeapOptimizationEnabled() && !isPhantom()) {
                        OffHeapIndexesValuesHandler.update(this, entryLayout.getIndexValuesBytes(cacheManager), cacheManager.getOffHeapByteCounter());
                    }
