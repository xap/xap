...
public class BlobStoreInitialLoadDataIterator implements ISAdapterIterator<IEntryHolder> {
    ...
    public IEntryHolder next() throws SAException {
        ...
        if (_engine.getCacheManager().isOffHeapOptimizationEnabled()) {
            try {
                long offHeapAddress = OffHeapIndexesValuesHandler.allocate(entryLayout.getIndexValuesBytes(_engine.getCacheManager()), oeh.getBlobStoreResidentPart().getOffHeapAddress(),_engine.getCacheManager().getBlobStoreInternalCache().getOffHeapByteCounter(), oeh.getBlobStoreResidentPart().getServerTypeDesc().getOffHeapTypeCounter());
