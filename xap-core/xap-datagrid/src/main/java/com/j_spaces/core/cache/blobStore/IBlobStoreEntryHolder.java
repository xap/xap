 ...
 * @since 9.8
 */
public interface IBlobStoreEntryHolder {
    void setBlobStoreResidentPart(BlobStoreRefEntryCacheInfo blobStoreResidentPart);

    BlobStoreRefEntryCacheInfo getBlobStoreResidentPart();

    IEntryHolder getLatestEntryVersion(CacheManager cacheManager, boolean attatchToMemory, Context attachingContext);

    short getBlobStoreVersion();

    void setBlobStoreVersion(short version);
