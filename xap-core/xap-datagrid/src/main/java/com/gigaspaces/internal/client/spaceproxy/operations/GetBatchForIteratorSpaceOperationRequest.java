...
public class GetBatchForIteratorSpaceOperationRequest extends SpaceOperationRequest<GetBatchForIteratorSpaceOperationResult> {
    ...
    private int _batchSize; //TODO change name
    ...
    public PartitionedClusterExecutionType getPartitionedClusterExecutionType() {
        ...
        if (_batchSize != Integer.MAX_VALUE)
    ...
    public boolean processPartitionResult(
            GetBatchForIteratorSpaceOperationResult remoteOperationResult,
            List<GetBatchForIteratorSpaceOperationResult> previousResults,
        ...
        if (remoteOperationResult.hasException()) {
            ...
            if (remoteOperationResult.getEntryPackets() != null) {
                ...
                if (previousResults.size() + 1 == numberOfPartitions) {
                    levels = new ArrayList<>(_entries.size());
                    int index = 0;
            ...
            if (_batchSize != DEFAULT_MAX_ENTRIES) {
                _batchSize -= Objects.requireNonNull(remoteOperationResult.getEntryPackets()).length;
            }
    ...
    private static final short FLAG_MAX_ENTRIES = 1;
    ...
    public void readExternal(ObjectInput in)
        ...
        if (flags != 0) {
            _batchSize = (flags & FLAG_MAX_ENTRIES) != 0 ? in.readInt() : DEFAULT_MAX_ENTRIES;
            _modifiers = (flags & FLAG_MODIFIERS) != 0 ? in.readInt() : DEFAULT_MODIFIERS;
        } else {
            _modifiers = DEFAULT_MODIFIERS;
            _batchSize = DEFAULT_MAX_ENTRIES;
        }
    }

    @Override
    public RemoteOperationRequest<GetBatchForIteratorSpaceOperationResult> createCopy(int targetPartitionId) {
        GetBatchForIteratorSpaceOperationRequest copy = (GetBatchForIteratorSpaceOperationRequest) super.createCopy(targetPartitionId);
