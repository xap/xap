...
public class SpaceGetBatchForIteratorExecutor extends SpaceActionExecutor{
    ...
    public SpaceResponseInfo execute(SpaceImpl space, SpaceRequestInfo spaceRequestInfo) {
        ...
        if(exception != null)
            return new SpaceIteratorBatchResult(null, partitionId, exception, requestInfo.getBatchNumber(), uuid);
        return new SpaceIteratorBatchResult(serverIteratorAnswerHolder.getEntryPackets(), partitionId, null, serverIteratorAnswerHolder.getBatchNumber(), uuid);
    }
