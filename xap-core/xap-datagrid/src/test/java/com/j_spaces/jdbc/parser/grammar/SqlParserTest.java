...
public class SqlParserTest {
    ...
    private final List<String> inputs = Arrays.asList(
            "SELECT REGION,ID_BB_PARSEKEY,EXCH_CODE,NAME,CRNCY,FLEX_ID,ID_BB_GLOBAL,ID_SEDOL1,ID_SEDOL2,SECURITY_TYP,EQY_SH_OUT,ID_BB_GLOBAL_COMPANY FROM com.gigaspaces.test.database.jdbc.aquarious_poc.generator.model.bloomberg.bloomberg_tzero_out WHERE REGION = ? AND DDATE = ?"
            ...
            , /*parsed successfully without changes */ "SELECT COUNTRY,CRNCY,ID_BB_GLOBAL,ID_BB_SEC_NUM_DES,EXCH_CODE,ID_SEDOL1,ID_SEDOL2,NAME,ID_BB_GLOBAL_COMPANY,AMT_OUTSTANDING,SECURITY_TYP  FROM com.gigaspaces.test.database.jdbc.aquarious_poc.generator.model.bloomberg_fi.corp_pfd_out WHERE REGION = ? AND DATA_DATE = ? AND MARKET_SECTOR_DES = 'Pfd' AND COUNTRY IN ('US', 'CA') AND CALLED_BOOL = 'N' AND EXCH_CODE <> 'NOT LISTED'"
            , /*parsed successfully without changes */ "SELECT btz.FLEX_ID, badj.CLOSE_PX_ADJ FROM com.gigaspaces.test.database.jdbc.aquarious_poc.generator.model.bloomberg.bloomberg_tzero_out btz, com.gigaspaces.test.database.jdbc.aquarious_poc.generator.model.bloomberg.bloomberg_tzero_px_out_adjusted badj WHERE btz.REGION = ? and btz.DDATE = ? AND btz.DDATE = badj.OPEN_DATE AND btz.ID_BB_GLOBAL = badj.ID_BB_GLOBAL"
