    ...
    private static class SystemBootShutdownHook extends Thread {
        ...
        private StringBuilder processShutdownHooks() {
            ...
            synchronized (shutdownHooks) {
                ...
                //process shutdown hooks in backwards order
                final ListIterator listIterator = shutdownHooks.listIterator(shutdownHooks.size());
                while (listIterator.hasPrevious()) {
                    Thread shutdownHookThread = (Thread) listIterator.previous();
                    Future<?> future = null;
                    try {
                        sb.append(dtf.format(LocalDateTime.now()))
                                .append("> call shutdown hook [")
                                .append(shutdownHookThread.getName())
                                .append("]\n");
                        future = singleThreadExecutor.submit(shutdownHookThread);
